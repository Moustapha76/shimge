...
export default function TableCustomized() {
 ...
 const { newsData } = React.useContext(GlobalContext);
        ...
        <div className='container'>
          {(rowsPerPage > 0
            ...
            <div className='article' key={row.id}>
              <div className='article-img'>
                  <img src={`${row.image}`} alt="Maison de luxe" />
              </div>
              <div className='dscription'>
                  <h4 className='title'>{row.title}</h4>
                  <div className='details'>
                      <span className='date'><CalendarOutlined />{row.pub}</span>
                      <a href={`/actualites/${row.id}`}>Lire +</a>
                  </div>
              </div>
            </div>
          ))}
          ...
          {emptyRows > 0 && (
            ...
            <CustomTablePagination
              rowsPerPageOptions={[6, 9, { label: 'All', value: -1 }]}
              colSpan={3}
